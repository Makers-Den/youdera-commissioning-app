...
export const EnergyConsumptionCommercial = () => {
  ...
  const methods = useForm<EnergyConsumptionType>({
    ...
    defaultValues: {
      ...
      openingDays: options.filter(option =>
        data.openingDays?.includes(option.value),
      ),
      openingTimes: {
        from: data.openingTimes?.from || 8,
        to: data.openingTimes?.to || 16,
      },
    },
  });
  ...
  return (
    ...
    >
      <Form
        ...
        {...methods}
      >
        <div className="md:max-w-container z-10 flex flex-col gap-7">
          <BodyText>
            {intl.formatMessage({
              defaultMessage: ` Please fill in the yearly kWh consumption of your commercial
            property.`,
            })}
          </BodyText>
          <InputField
          ...
          <NoteText>
            {intl.formatMessage({
              defaultMessage:
                'For properties with consumption larger than 2 mWh we suggest reaching out to sales directly.',
            })}
          </NoteText>
          <HorizontalSelectField
            name="openingDays"
            label={intl.formatMessage({
              defaultMessage: 'Regular opening days',
            })}
            options={options}
          />
